...
using Owin;
using NTUOC7registration.Models;
using System.Data.Entity;

namespace NTUOC7registration.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                var user = await UserManager.FindAsync(model.unicode, model.Password);
                if (user != null)
                {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser() { UserName = model.unidcode, Email = model.Email,name=model.name,department=model.department,sex=model.sex,telphone=model.telphone,mobile=model.mobile,parent=model.parent,relation=model.relation,emergence=model.emergence,dorm=model.dorm,sleepbag=model.sleepbag, food=model.food,eatother=model.eatother,disease=model.disease,disother=model.disother,lowincomes=model.lowincomes,size=model.size,other=model.other};
        ...
        public ActionResult Manage(ManageMessageId? message)
        {
            ...
            model.telphone = currentUser.telphone;
            model.department = currentUser.department;
            model.mobile = currentUser.mobile;
            model.Email = currentUser.Email;
            model.parent = currentUser.parent;
            model.relation = currentUser.relation;
            model.emergence = currentUser.emergence;
            model.dorm = currentUser.dorm;
            model.sleepbag = currentUser.sleepbag;
            model.food = currentUser.food;
            model.eatother = currentUser.eatother;
            model.disease = currentUser.disease;
            model.disother = currentUser.disother;
            model.lowincomes = currentUser.lowincomes;
            model.size = currentUser.size;
            model.other = currentUser.other;
            return View(model);
        }

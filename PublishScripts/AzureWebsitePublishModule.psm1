...
<#
.SYNOPSIS
以指令碼範圍中的 $Script:originalSubscription 變數儲存目前的 Windows Azure 訂用帳戶。

.DESCRIPTION
Backup-Subscription 函式會將目前的 Windows Azure 訂用帳戶 (Get-AzureSubscription -Current) 及其儲存體帳戶，以及這個指令碼 ($UserSpecifiedSubscription) 所變更的訂用帳戶及其儲存體帳戶儲存在指令碼範圍中。透過儲存值，您就可以使用 Restore-Subscription 等函式，將原始的目前訂用帳戶和儲存體帳戶還原為目前狀態 (如果目前狀態已變更)。
...
<#
.SYNOPSIS
還原為 "current" 狀態的 Windows Azure 訂用帳戶，該帳戶是以指令碼範圍中的 $Script:originalSubscription 變數來儲存。

.DESCRIPTION
Restore-Subscription 函式會將以 $Script:originalSubscription 變數儲存的訂用帳戶設成目前的訂用帳戶 (再次)。如果原始訂用帳戶具有儲存體帳戶，則這個函式會針對目前訂用帳戶將該儲存體帳戶設成目前的帳戶。函式只會在環境中有非 null 的 $SubscriptionName 變數時才會還原訂用帳戶。否則，就會結束。如果填入 $SubscriptionName 但 $Script:originalSubscription 是 $null，則 Restore-Subscription 會使用 Select-AzureSubscription Cmdlet 來清除 Windows Azure PowerShell 中訂用帳戶的目前及預設設定。這個函式沒有參數，因此不會接受任何輸入，而且不會傳回任何項目 (void)。您可以使用 -Verbose 將訊息寫入 Verbose 資料流。
...
-- 它一開始會將 JSON 檔案轉換成 PSCustomObject。網站雜湊表有下列索引鍵:
-- Location: 網站位置
...
.DESCRIPTION
使用特定名稱和位置建立 Windows Azure 網站。這個函式會呼叫 Azure 模組中的 New-AzureWebsite Cmdlet。如果訂用帳戶還沒有所指定名稱的網站，這個函式會建立網站並傳回網站物件。否則，傳回現有網站。

.PARAMETER  Name
指定新網站的名稱。名稱在 Windows Azure 中必須是唯一的。這個參數是必要項。

.PARAMETER  Location
指定網站的位置。有效值為 Windows Azure 位置，例如「美國西部」。這個參數是必要項。
...
<#
.SYNOPSIS
建立可讓您連接到 Windows Azure SQL 資料庫的字串。

.DESCRIPTION
Get-AzureSQLDatabaseConnectionString 函式會組譯連接字串，以連接到 Windows Azure SQL 資料庫。

.PARAMETER  DatabaseServerName
指定 Windows Azure 訂用帳戶中現有資料庫伺服器的名稱。所有 Windows Azure SQL 資料庫都必須與 SQL 資料庫伺服器產生關聯。若要取得伺服器名稱，請使用 Get-AzureSqlDatabaseServer Cmdlet (Azure 模組)。這個參數是必要項。
...
<#
.SYNOPSIS
從 Visual Studio 所產生 JSON 組態檔中的值建立 Windows Azure SQL 資料庫。
...
指定 SQL Database 伺服器管理員的密碼。輸入內含 Name 和 Password 索引鍵的雜湊表。Name 的值是 SQL Database 伺服器的名稱。Password 的值是管理員的密碼。例如: @Name = "TestDB1"; Password = "password"。這個參數是選擇性的。如果省略，或是 SQL Database 伺服器名稱不符合 $DatabaseConfig 物件的 serverName 屬性值，則函式會使用連接字串中 SQL Database 的 $DatabaseConfig 物件的 Password 屬性。
...
<#
.SYNOPSIS
建立新的 Windows Azure SQL 資料庫。

.DESCRIPTION
Add-AzureSQLDatabase 會從 Visual Studio 所產生 JSON 組態檔中的資料建立 Windows Azure SQL 資料庫，並且傳回新的資料庫。如果訂用帳戶在指定的 SQL 資料庫伺服器中已有具有指定之資料庫名稱的 SQL 資料庫，函式就會傳回現有資料庫。這個函式會呼叫 New-AzureSqlDatabase Cmdlet (Azure 模組)，實際上，此 Cmdlet 會建立 SQL 資料庫。
